...
VALUES
  (
      ...
      0,
      '获取${table.comment!}树',
      '${config.serviceName?replace("-",":")}:<#if config.controllerPath?? && config.controllerPath != "">${config.controllerPath?replace("/","","f")?replace("/",":")}<#else>/${table.entityPath}</#if>:tree',
      '4',
      'xitongrizhi',
      '${config.serviceName?replace("-","/")}/<#if config.controllerPath?? && config.controllerPath != "">${config.controllerPath?replace("/","","f")}<#else>/${table.entityPath}</#if>/tree',
