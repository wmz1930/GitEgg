...
<#list table.commonFields as field><#--生成公共字段 -->
    ...
    <!-- 联合主表通用查询映射结果 -->
    <resultMap id="${mainEntityName}${entity}BaseResultMap" type="${package.Entity?replace("entity","dto")}.${entity}DTO" extends="${mainPackagePath}.mapper.${mainEntityName}Mapper.BaseResultMap">
<#list table.fields as field>
    <#if !field.keyFlag><#--生成普通字段 -->
        <result column="${field.name}" property="${field.propertyName}" />
    </#if>
...
<#if baseColumnList>
    ...
    <sql id="${entity}Base_Column_List">
        <#list table.fields as field> ${config.tableAlias}.${field.name} as ${field.name}<#if field?has_next>,</#if></#list>
    </sql>
...
<#assign dtoPackage="${package.Entity}"/>
    ...
    <select id="query${mainEntityName}${entity}List" resultType="${dtoPackage?replace("entity","dto")}.${entity}DTO" parameterType="${dtoPackage?replace("entity","dto")}.Query${entity}DTO">
        SELECT
...
<#if field?? && field.queryTerm == true>
    ...
    <select id="query${mainEntityName}${entity}" resultType="${dtoPackage?replace("entity","dto")}.${entity}DTO" parameterType="${dtoPackage?replace("entity","dto")}.Query${entity}DTO">
        SELECT
        <include refid="${entity}Base_Column_List"/>,
        <include refid="${mainPackagePath}.mapper.${mainEntityName}Mapper.${mainEntityName}Base_Column_List"/>
        FROM ${mainTable} ${mainTableAlias}
