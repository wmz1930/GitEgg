...
(
        ${maxId},
        0,
        '联合主表获取${table.comment!}列表',
        '${config.serviceName?replace("-",":")}:<#if config.controllerPath?? && config.controllerPath != "">${config.controllerPath?replace("/","","f")?replace("/",":")}<#else>/${table.entityPath}</#if>:main:list',
        '4',
        'xitongrizhi',
        '${config.serviceName?replace("-","/")}/<#if config.controllerPath?? && config.controllerPath != "">${config.controllerPath?replace("/","","f")}<#else>/${table.entityPath}</#if>/main/list',
        '<#if config.serviceName?? && config.serviceName != "">/${config.serviceName}</#if><#if config.controllerPath?? && config.controllerPath != "">${config.controllerPath}<#else>/${table.entityPath}</#if>/main/list',
        2,
        1,
        1,
        NULL,
        '1',
        '联合主表获取${table.comment!}列表数据',
        NOW(),
        1,
        NOW(),
        1,
