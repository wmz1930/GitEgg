...
public class ${table.controllerName} {
     ...
     */
     ...
     public Result<Page<${entity}DTO>> mainList(@RequestBody Query${entity} query${entity}) {
         Page<${entity}DTO> page${entity} = ${table.entityPath}Service.query${mainEntityName}${entity}List(query${entity}.getPage(), query${entity}.getQuery${entity}DTO());
...
<#if checkExist?? && checkExist == true>
    ...
    */
    ...
    public void download(HttpServletResponse response, Query${entity} query${entity}) throws IOException {
        response.setContentType("application/vnd.ms-excel");
        response.setCharacterEncoding("utf-8");
        // 这里URLEncoder.encode可以防止中文乱码 当然和easyexcel没有关系
        String fileName = URLEncoder.encode("${table.comment!}数据列表", "UTF-8").replaceAll("\\+", "%20");
        response.setHeader("Content-disposition", "attachment;filename*=utf-8''" + fileName + ".xlsx");
        List<${entity}DTO> ${table.entityPath}List = ${table.entityPath}Service.query${mainEntityName}${entity}List(query${entity}.getQuery${entity}DTO());
