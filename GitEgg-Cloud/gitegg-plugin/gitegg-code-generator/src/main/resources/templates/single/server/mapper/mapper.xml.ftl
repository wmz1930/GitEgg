...
<#assign dtoPackage="${package.Entity}"/>
    ...
    <#elseif field?? && field.queryType == "LIKE_RIGHT">
        ...
        <#if tableShowType?? && tableShowType == "table_table" && !(hasParentId?? && hasParentId == true)>
            AND parent_id = <#noparse>#{</#noparse>${table.entityPath}.parentId<#noparse>}</#noparse>
        </#if>
        <#------------  END 字段循环遍历  ---------->
        ...
        <if test="${table.entityPath}.parentId != null and ${table.entityPath}.parentId != ''">
            <choose>
                ...
                <when test="${table.entityPath}.isLeaf != null and ${table.entityPath}.isLeaf == '1'.toString()">
                    AND find_in_set(<#noparse>#{</#noparse>${table.entityPath}.parentId<#noparse>}</#noparse>, ancestors)
                </when>
                <!-- 查询子${table.comment!}，包含parentId${table.comment!}本身 -->
                <otherwise>
                    AND (id = <#noparse>#{</#noparse>${table.entityPath}.parentId<#noparse>}</#noparse> OR find_in_set(<#noparse>#{</#noparse>${table.entityPath}.parentId<#noparse>}</#noparse>, ancestors))
                </otherwise>
            </choose>
        </if>
        <#-- ----------  BEGIN 字段循环遍历 ---------->
