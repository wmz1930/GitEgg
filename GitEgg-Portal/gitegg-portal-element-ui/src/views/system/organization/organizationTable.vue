<template>
  <div class="app-container">
    ...
    <el-table :data="list" row-key="id" border>
      ...
      <el-table-column :label="$t('organizationTable.organizationStatus')" class-name="status-col" width="100">
        <template slot-scope="scope">
          <el-tag :type="scope.row.organizationStatus | statusFilter">{{ scope.row.organizationStatus | statusNameFilter }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column :label="$t('table.actions')" width="300">
        <template slot-scope="scope">
          ...
          <el-button v-if="scope.row.organizationStatus!='1'" size="mini" type="success" @click="handleModifyStatus(scope.row,'1')">{{ $t('organizationTable.enable') }}
          </el-button>
          <el-button v-if="scope.row.organizationStatus!='0'" size="mini" @click="handleModifyStatus(scope.row,'0')">{{ $t('organizationTable.disable') }}
          </el-button>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="organizationForm" :model="organizationForm" :rules="rules" label-width="100px" class="organizationForm" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item :label="$t('organizationTable.organizationStatus')" prop="organizationStatus">
        ...
        <el-form-item :label="$t('organizationTable.comments')">
          <el-input v-model.trim="organizationForm.comments" :autosize="{ minRows: 2, maxRows: 4}" type="textarea" placeholder="请输入备注信息" />
        </el-form-item>
      </el-form>
...
<script>
import { fetchOrgList, createOrganization, deleteOrganization, updateOrganization, updateOrganizationStatus, checkOrganizationExist } from '@/api/system/organization'
