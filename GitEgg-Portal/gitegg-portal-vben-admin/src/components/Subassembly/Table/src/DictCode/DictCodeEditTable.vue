<template>
  ...
  <!-- 新增编辑时的弹框 -->
  <DictCodeModal @register="registerModal" @success="handleSuccess" />
</template>
<script lang="ts">
  import { defineComponent, reactive, ref } from 'vue';
  import { BasicTable, useTable, TableAction } from '/@/components/Table';
  import {
    getGeneratorDictList,
    deleteGeneratorDict,
    batchDeleteGeneratorDict,
  } from '/@/api/plugin/codeGenerator/dict/dict';
  import { useModal } from '/@/components/Modal';
  import DictCodeModal from './DictCodeModal.vue';

  import { columns, getSearchFormConfig } from './dict.code';
  ...
  export default defineComponent({
    ...
    setup(_, { attrs, emit }) {
      ...
      const [registerModal, { openModal: openDictCodeModal }] = useModal();
      const searchInfo = reactive<Recordable>(attrs.searchInfo as Recordable);
      const createText = ref(attrs?.createText ? attrs?.createText : '新建基础配置字典');
      // 当前父节点
      let currentParentInfo = ref();

      const [registerTable, { reload, updateTableDataRecord }] = useTable({
        api: getGeneratorDictList,
        rowKey: 'id',
        columns,
        useSearchForm: true,
      ...
      function handleDelete(record: Recordable) {
        deleteGeneratorDict(record.id)
          .then(() => {
            createMessage.success(`基础配置字典删除成功`);
            reload();
          })
          .catch(() => {
            createMessage.error('基础配置字典删除失败');
          })
          .finally(() => {});
      }

      function handleBatchDelete() {
        batchDeleteGeneratorDict(checkedKeys.value)
          .then(() => {
            createMessage.success(`基础配置字典删除成功`);
            checkedKeys.value = [];
            reload();
      ...
      function handleDictRecord(record: Recordable) {
        // 基础配置字典配置的都是字符串，这个在后台返回基础配置字典状态Number类型时需要进行转换
