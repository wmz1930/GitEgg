<template>
  <div>
    ...
    >
      <template #tableTitle>
        <a-button type="primary" @click="handleCreate" class="mr-2">
          <icon icon="ant-design:plus-outlined" /> 新建短信渠道
        </a-button>
        <Dropdown
    ...
    <SmsChannelDrawer @register="registerDrawer" @success="handleSuccess" />
  </div>
</template>
<script lang="ts">
  ...
  import {
    querySmsChannelList,
    deleteSmsChannel,
    batchDeleteSmsChannel,
    downloadSmsChannelList,
    // importSmsTemplateList,
    // downloadSmsTemplateTemplate,
  } from '/@/api/system/extension/sms/smsChannel';

  import { useDrawer } from '/@/components/Drawer';
  import SmsChannelDrawer from './SmsChannelDrawer.vue';
  // import SmsTemplateResourceDrawer from './SmsTemplateResourceDrawer.vue';

  import { smsChannelColumns, channelSearchForm } from './sms.channel.data';
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const [registerTable, { reload, updateTableDataRecord, getForm }] = useTable({
        title: '短信渠道列表',
        api: querySmsChannelList,
        rowKey: 'id',
        columns: smsChannelColumns,
        formConfig: {
          labelWidth: 120,
          schemas: channelSearchForm,
        },
        useSearchForm: true,
      ...
      function handleDelete(record: Recordable) {
        deleteSmsChannel(record.id)
          .then(() => {
            createMessage.success(`短信模板删除成功`);
            reload();
          })
          .catch(() => {
            createMessage.error('短信模板删除失败');
          })
          .finally(() => {});
      }

      function handleBatchDelete() {
        batchDeleteSmsChannel(checkedKeys.value)
          .then(() => {
            createMessage.success(`短信模板删除成功`);
            checkedKeys.value = [];
            reload();
      ...
      //   openFullLoading();
      //   importSmsTemplateList(formData)
      //     .then(() => {
      ...
      //   openFullLoading();
      //   downloadSmsTemplateTemplate(searchInfo)
      //     .then((response) => {
