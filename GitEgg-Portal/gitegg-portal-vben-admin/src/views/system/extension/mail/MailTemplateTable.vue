<template>
  <div>
    ...
    >
      <template #tableTitle>
        <a-button type="primary" @click="handleCreate" class="mr-2">
          <icon icon="ant-design:plus-outlined" /> 新增邮件模板
        </a-button>
        <Dropdown
    ...
    <MailTemplateDrawer @register="registerDrawer" @success="handleSuccess" />
  </div>
</template>
<script lang="ts">
  ...
  import {
    queryMailTemplateList,
    deleteMailTemplate,
    batchDeleteMailTemplate,
    downloadMailTemplateList,
    uploadMailTemplate,
    downloadMailTemplateTemplate,
  } from '/@/api/system/extension/mail/mailTemplate';

  import { useDrawer } from '/@/components/Drawer';
  import MailTemplateDrawer from './drawer/MailTemplateDrawer.vue';
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const [registerTable, { reload, updateTableDataRecord, getForm }] = useTable({
        title: '邮件模板列表',
        api: queryMailTemplateList,
        rowKey: 'id',
        columns: mailTemplateColumn,
        formConfig: {
          autoAdvancedLine: 1,
          labelWidth: 120,
          schemas: mailTemplateSearch,
        },
        useSearchForm: true,
      ...
      function handleDelete(record: Recordable) {
        deleteMailTemplate(record.id)
          .then(() => {
            createMessage.success(`邮件模板删除成功`);
            reload();
          })
          .catch(() => {
            createMessage.error('邮件模板删除失败');
          })
          .finally(() => {});
      }

      function handleBatchDelete() {
        batchDeleteMailTemplate(checkedKeys.value)
          .then(() => {
            createMessage.success(`邮件模板删除成功`);
            checkedKeys.value = [];
            reload();
      ...
      function handleSmsTemplateRecord(record: Recordable) {
        ...
        record.templateType = `${record.templateType}`;
      }
      ...
      function handleExportSmsTemplateList(exportChecked: boolean) {
        ...
        downloadMailTemplateList(params)
          .then((response) => {
            handleDownloadBlod('邮件模板数据列表.xlsx', response);
            closeFullLoading();
          })
          .catch((err) => {
            createMessage.error('邮件模板数据导出失败:' + err);
          })
          .finally(() => {
            closeFullLoading();
          });
      ...
      function handleUploadSmsTemplateList(file) {
        ...
        openFullLoading();
        uploadMailTemplate(formData)
          .then(() => {
            createMessage.success(`邮件模板数据导入成功`);
            reload();
          })
          .catch((err) => {
            createMessage.error(`邮件模板数据导入失败: ` + err);
          })
          .finally(() => {
            closeFullLoading();
          });
      ...
      function handleDownloadTemplate() {
        openFullLoading();
        downloadMailTemplateTemplate(searchInfo)
          .then((response) => {
            handleDownloadBlod('邮件模板模板批量导入模板.xlsx', response);
          })
          .catch((err) => {
