...
const allowList = ['login', 'register', 'recover', 'registerResult', 'socialCallback', 'socialBind', 'registerAccount'] // no redirect allowList
    ...
    if (to.path === loginRoutePath) {
      ...
      if (store.getters.roles.length === 0) {
        ...
        store
          .dispatch('GetInfo')
          .then(async res => {
            const roles = res.data && res.data.roleIdList
