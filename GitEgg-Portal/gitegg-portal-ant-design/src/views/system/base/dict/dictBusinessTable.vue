...
import { fetchDictBusinessList, createDictBusiness, updateDictBusiness, deleteDictBusiness, batchDeleteDictBusiness, updateDictBusinessStatus, checkDictBusinessExist } from '@/api/system/base/dictBusiness'
...
export default {
  ...
  data () {
    var validDictCode = (rule, value, callback) => {
      ...
      checkDictBusinessExist(keyData).then(response => {
        if (!response.data) {
  ...
  methods: {
    ...
    createData () {
      this.$refs['dictForm'].validate(valid => {
        if (valid) {
          createDictBusiness(this.dictForm).then(response => {
    ...
    updateData () {
      this.$refs['dictForm'].validate(valid => {
        if (valid) {
          updateDictBusiness(this.dictForm).then(() => {
            this.dialogFormVisible = false
    ...
    handleDelete (row, rootFlag) {
      ...
      this.$confirm({
        ...
        content: '',
        onOk () {
          deleteDictBusiness(row.id).then(() => {
    ...
    handleBatchDelete (rootFlag) {
      ...
      that.$confirm({
        ...
        onOk () {
          ...
          batchDeleteDictBusiness(deleteIds).then(() => {
    ...
    handleModifyStatus (row, status) {
      ...
      updateDictBusinessStatus(row.id, status).then(() => {
