<template>
  <a-card :bordered="false"
        ...
        <a-row :gutter="48">
          <a-col :md="6"
            ...
            <a-form-model-item label="接口名称">
              <a-input v-model="listQuery.methodName"
                       placeholder="接口名称"
                       class="filter-item"
                       :maxLength="32"
          ...
          <a-col :md="6"
            ...
            <a-form-model-item label="操作人">
              <a-input v-model="listQuery.creator"
                       placeholder="操作人"
                       class="filter-item"
                       :maxLength="32"
                       @keyup.enter.native="handleFilter" />
          ...
          <a-col :md="6"
            ...
            <a-form-model-item label="日志类型">
              <a-select v-model="listQuery.logType"
                        placeholder="日志类型"
                        allow-clear>
                <a-select-option v-for="item in typeOption"
                                 :key="item.key"
                                 :value="item.key">
                  {{ item.label }}
                </a-select-option>
          ...
          <template v-if="advanced">
            ...
            <a-col :md="6"
              ...
              <a-form-model-item label="开始时间">
                <a-date-picker v-model.trim="listQuery.startTime"
                               placeholder="开始时间"
            ...
            <a-col :md="6"
              ...
              <a-form-model-item label="结束时间">
                <a-date-picker v-model.trim="listQuery.endTime"
                               placeholder="结束时间"
                               valueFormat="YYYY-MM-DD HH:mm:ss"
                               style="width:100%;" />
              </a-form-model-item>
    ...
    <s-table ref="logTable"
      ...
      <span slot="inParams"
        ...
        <span class="link-type"
              @click="handleInShow(record.inParams)">{{ record.inParams }}</span>
      </span>
      <span slot="outParams"
            slot-scope="text, record">
        <span class="link-type"
              @click="handleOutShow(record.outParams)">{{ record.outParams }}</span>
      </span>
      <span slot="logType"
            slot-scope="text, record">
    ...
    <a-modal :visible.sync="dialogFormVisible"
