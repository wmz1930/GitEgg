  ...
  <div>
    <a-form :form="form" style="max-width: 500px; margin: 40px auto 0;">
      ...
      <a-form-item :wrapperCol="{span: 24}">
        <span class="has-account" style="float:left;color: #52c41a;"><a-icon type="bell" /> 未注册的手机号验证后自动创建账号</span>
        <a-button type="primary" @click="bindMobile" style="float:right;">同意协议并绑定</a-button>
      </a-form-item>
    </a-form>
...
export default {
  ...
  data () {
    var validSmsCode = (rule, value, callback) => {
      if (this.form.getFieldValue('phoneNumber') && value && value.length === 6) {
        ...
        checkSmsCodePre(params).then(response => {
          if (!response.data) {
  ...
  methods: {
    ...
    bindMobile () {
      ...
      validateFields((err, values) => {
        if (!err) {
          ...
          bindParams.code = values.captcha
          bindParams.socialKey = this.$route.query.key
          // 执行绑定操作
          userBindMobile(bindParams).then(res => {
            // 绑定成功后，调用第三方登录方法
            this.$emit('bindLogin')
          }).catch(err => {
    ...
    verifySendSmsSuccess (params) {
      ...
      validateFields(validateFieldsKey, { force: true }, (err, values) => {
        if (!err) {
          ...
          registerParams.smsCode = 'sms_login_code'
          // 判断是图片验证码还是滑动验证码
          if (this.loginCaptchaType === 'sliding') {
    ...
    getCaptcha (registerParams) {
      ...
      sendSmsCode(serialize(registerParams)).then(res => {
        setTimeout(hide, 1)
