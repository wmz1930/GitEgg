<template>
  <a-card :bordered="false" class="content">
    ...
    <s-table
      ref="codeGenerationDatasourceTable"
      size="default"
      bordered
      :rowKey="row=>row.id"
      :columns="columns"
      ...
      :scroll="{x:1500}"
      showPagination="auto"
      :pagination="codeGenerationDatasourcePagination"
    ...
    <a-modal :title="textMap[dialogStatus]" :maskClosable="false" :visible="dialogFormVisible" :width="800" @cancel="() => dialogFormVisible = false">
      <a-form-model
        ref="codeGenerationDatasourceForm"
        :model="codeGenerationDatasourceForm"
        :rules="rules"
        :label-col="codeGenerationDatasourceLabelCol"
        :wrapper-col="codeGenerationDatasourceWrapperCol">
        <a-form-model-item label="数据源名称" prop="datasourceName">
          <a-input v-model="codeGenerationDatasourceForm.datasourceName" placeholder="输入数据源名称" :maxLength="64" />
        </a-form-model-item>
        <a-form-model-item label="连接地址" prop="url">
          <a-input v-model="codeGenerationDatasourceForm.url" placeholder="输入连接地址" :maxLength="500" />
        </a-form-model-item>
        <a-form-model-item label="用户名" prop="username">
          <a-input v-model="codeGenerationDatasourceForm.username" placeholder="输入用户名" :maxLength="64" />
        </a-form-model-item>
        <a-form-model-item label="密码" prop="password">
          <a-input v-model="codeGenerationDatasourceForm.password" placeholder="输入密码" :maxLength="64" />
        </a-form-model-item>
        <a-form-model-item label="数据库驱动" prop="driver">
          <a-input v-model="codeGenerationDatasourceForm.driver" placeholder="输入数据库驱动" :maxLength="64" />
        </a-form-model-item>
        <a-form-model-item label="数据库类型" prop="dbType">
          <a-select v-model="codeGenerationDatasourceForm.dbType" placeholder="数据库类型" show-search :filter-option="filterOption">
            <a-select-option v-for="item in dbTypeDictList" :key="item.dictCode" :value="item.dictCode">
              {{ item.dictName }}
            </a-select-option>
          </a-select>
        ...
        <a-form-model-item label="备注" prop="comments">
          <a-input v-model="codeGenerationDatasourceForm.comments" placeholder="输入备注" :maxLength="255" />
        </a-form-model-item>
      </a-form-model>
    ...
    export default {
        ...
        data () {
            var validDatasourceName = (rule, value, callback) => {
                var keyData = {
                    id: this.codeGenerationDatasourceForm.id,
                    checkField: 'datasource_name',
                    checkValue: value
                }
                checkCodeGenerationDatasourceExist(keyData).then(response => {
                    if (!response.data) {
            ...
            return {
                currentCodeGenerationDatasource: '',
                filterText: '',
                tableKey: 0,
                list: null,
                total: 0,
                ...
                codeGenerationDatasourceLabelCol: {
                    xs: { span: 24 },
                    sm: { span: 5 }
                },
                codeGenerationDatasourceWrapperCol: {
                    xs: { span: 24 },
                    sm: { span: 16 }
                },
                selectedRowKeys: [],
                selectedRows: [],
                codeGenerationDatasourcePagination: {
                    defaultPageSize: 10,
                ...
                loadData: parameter => {
                    return queryCodeGenerationDatasourceList(Object.assign(parameter, this.listQuery))
                        .then(res => {
                            this.list = res.data
                            return res
                        })
        ...
        watch: {
            // filterText (val) {
            //   this.$refs['codeGenerationDatasourceTree'].filter(val)
            // }
        },
        created () {
        ...
        methods: {
          queryDbTypeDictList () {
            ...
            resetCodeGenerationDatasourceForm () {
                this.codeGenerationDatasourceForm = {
                    id: '',
                    datasourceName: '',
                    url: '',
                    username: '',
            ...
            getList () {
                ...
                queryCodeGenerationDatasourceList(this.listQuery).then(response => {
            ...
            handleFilter () {
                this.$refs.codeGenerationDatasourceTable.refresh(true)
            },
            handleTableRefresh () {
                this.$refs.codeGenerationDatasourceTable.refresh()
            },
            handleCreate () {
                this.resetCodeGenerationDatasourceForm()
                this.dialogStatus = 'create'
                this.dialogFormVisible = true
                this.$nextTick(() => {
                    this.$refs['codeGenerationDatasourceForm'].clearValidate()
                })
            },
            createData () {
                this.$refs['codeGenerationDatasourceForm'].validate(valid => {
                    if (valid) {
                        createCodeGenerationDatasource(this.codeGenerationDatasourceForm).then(() => {
            ...
            handleUpdate (row) {
                this.codeGenerationDatasourceForm = Object.assign({}, row) // copy obj
            ...
            updateData () {
                this.$refs['codeGenerationDatasourceForm'].validate(valid => {
                    if (valid) {
                        updateCodeGenerationDatasource(this.codeGenerationDatasourceForm).then(() => {
            ...
            handleDelete (row) {
                ...
                this.$confirm({
                    ...
                    onOk () {
                        that.listLoading = true
                        deleteCodeGenerationDatasource(row.id).then(() => {
                            that.listLoading = false
            ...
            handleBatchDelete () {
                ...
                var codeGenerationDatasourceList = this.selectedRows.map(function (n) {
                    return n.id
                })
                var that = this
                this.$confirm({
                    title: '以下选中记录将被全部删除，是否继续?',
                    content: codeGenerationDatasourceList.join(','),
                    onOk () {
                        that.listLoading = true
                        batchDeleteCodeGenerationDatasource(that.selectedRowKeys).then(() => {
                            that.listLoading = false
                            that.$message.success('删除成功!')
