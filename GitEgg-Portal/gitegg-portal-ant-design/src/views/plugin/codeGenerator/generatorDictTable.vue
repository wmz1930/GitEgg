...
import { fetchGeneratorDictList, createGeneratorDict, updateGeneratorDict, deleteGeneratorDict, batchDeleteGeneratorDict, updateGeneratorDictStatus, checkGeneratorDictExist } from '@/api/plugin/codeGenerator/dict/dict'
...
export default {
  ...
  data () {
    var validDictCode = (rule, value, callback) => {
      ...
      checkGeneratorDictExist(keyData).then(response => {
        if (!response.data) {
    ...
    return {
      ...
      loadParentData: parameter => {
        return fetchGeneratorDictList(Object.assign(parameter, this.parentTreeQuery))
          .then(res => {
      ...
      // 加载数据方法 必须为 Promise 对象
      loadData: parameter => {
        return fetchGeneratorDictList(Object.assign(parameter, this.treeQuery))
          .then(res => {
            this.list = res.data
            return res
          })
  ...
  methods: {
    ...
    createData () {
      this.$refs['dictForm'].validate(valid => {
        if (valid) {
          createGeneratorDict(this.dictForm).then(response => {
    ...
    updateData () {
      this.$refs['dictForm'].validate(valid => {
        if (valid) {
          updateGeneratorDict(this.dictForm).then(() => {
            this.dialogFormVisible = false
    ...
    handleDelete (row, rootFlag) {
      ...
      this.$confirm({
        ...
        content: '',
        onOk () {
          deleteGeneratorDict(row.id).then(() => {
    ...
    handleBatchDelete (rootFlag) {
      ...
      that.$confirm({
        ...
        onOk () {
          ...
          batchDeleteGeneratorDict(deleteIds).then(() => {
    ...
    handleModifyStatus (row, status) {
      ...
      updateGeneratorDictStatus(row.id, status).then(() => {
